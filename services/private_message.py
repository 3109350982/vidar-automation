    ...
    async def execute(self, message_template="您好，看到您的评论，很高兴认识您！", duration_minutes=10, user_urls=None, **kwargs):
        ...
        try:
            ...
            for user in pending_users:
                ...
                # 🔁 轮询场景：仍插入极短节律，避免连续爆发
                await self.pause(8, 15, 'message_send')
                ...
                # 智能延迟，避免操作过于频繁
                # 智能延迟，避免操作过于频繁
                await self._emit_event("operation", f"⏳ 等待 {interval_minutes} 秒（刷视频中）")
                await self._browse_videos_for(interval_minutes , like_probability)

                if rotate_accounts and profile_dirs:
                    # 多账号轮询：本轮先完成间隔期刷视频，下一轮循环再切换到下一个账号
                    await self._emit_event(
                        "operation",
                        "🔁 多账号轮询已启用：已刷完本账号间隔期视频，下一位将切换到下一个账号"
                    )
                ...
                if total_messages_sent % 5 == 0 and total_messages_sent > 0:
                    from utils.advanced_anti_detection import anti_detection
                    await anti_detection.smart_rest(30, 50)
